# Faxbot

<p align="center">
  <img src="assets/images/faxbot_full_logo.png" alt="Faxbot" width="1000" height="1000" />
</p>

Open‑source, self‑hostable fax API with modular backends (Phaxio, Sinch, SIP/Asterisk), Admin Console, and MCP integration.

- Modern REST API for fax transmission
- HIPAA‑aligned controls and storage options
- Admin Console with mobile‑first UI
- SDKs for Node.js and Python

## Quick Links

- Backends: [Phaxio](go-live/phaxio.md) · [Sinch](go-live/sinch.md) · [SIP/Asterisk](go-live/sip-asterisk.md)
- API Reference: https://faxbot.net/api/v1/ · Swagger: https://faxbot.net/api/v1/swagger
- SDKs: [Node & Python](sdks/index.md)
- AI Integration: [MCP](mcp/index.md)
